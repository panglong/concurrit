                                    ...
                                    apr_pool_t *pool_to_recycle)
{
    ...
    /* NAJ: RACY, but designed to capture all threads but one worker */ 
    pid_t my_tid = syscall(__NR_gettid);

    if (lucky_one_tid == 0) {
        lucky_one_tid = my_tid;
    }

    while (my_tid != lucky_one_tid) {
        usleep(100000);
    }
                                          ...
                                          apr_pool_t **recycled_pool)
{
    ...
    *recycled_pool = NULL;
    ...
    if (queue_info->idlers < 0)  {
        fprintf(stderr, "WARNING, idlers is now %d\n", queue_info->idlers);
    }
