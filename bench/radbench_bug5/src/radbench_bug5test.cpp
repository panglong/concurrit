...
CONCURRIT_BEGIN_TEST(MyScenario, "MyScenario")
	...
	TEST(SearchContextBounded) {
		...
		RUN_THREAD_THROUGH(t2, RETURNS(), "Run t2");

		RUN_THREAD_THROUGH(t1, RETURNS(), "Run t1");
	}
	...
	TEST(ExactSchedule) {
		...
		RUN_THREAD_THROUGH(t2, RETURNS(), "Run t2 until returns");

		RUN_THREAD_THROUGH(t1, RETURNS(), "Run t1 until returns");
	}
